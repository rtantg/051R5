...
if (!$link) {
    ...
    // Selecteer alle reeds gespeelde wedstrijdn zonder score
    // ter informatie aan gebruiker dat scores nog moeten worden
    // doorgevoerd.
    $query1 = 'SELECT * FROM competitieschema
                WHERE datum <= CURDATE() 
                AND thuis_score IS NULL 
                AND uit_score IS NULL';

    // Selecteer alle gespeelde wedstrijden met score
    // voor berekening van stand en uitslagen.
    $query2 = 'SELECT a.*, b.logo, c.logo
               ...
               ORDER BY
                  a.datum';
    // Selecteer alle nog niet gespeelde wedstrijden voor het programma
    $query3 = 'SELECT  a.*, b.logo,c.logo
           FROM competitieschema AS a
    ...
    if ($result2 == false){
        ...
        if(!mysqli_num_rows($result2) ) {
            ...
            while($row=mysqli_fetch_row($result2)){
                $stand[$row[1]]['gespeeld'] = 0;
                $stand[$row[1]]['doelpunten'] = 0;
                $stand[$row[1]]['tegen'] = 0;
                $stand[$row[1]]['gewonnen'] = 0;
                $stand[$row[1]]['verloren'] = 0;
                $stand[$row[1]]['gelijk'] = 0;
                $stand[$row[1]]['punten'] = 0;
                $stand[$row[1]]['logo'] = '';

                $stand[$row[2]]['gespeeld'] = 0;
                $stand[$row[2]]['doelpunten'] = 0;
                $stand[$row[2]]['tegen'] = 0;
                $stand[$row[2]]['gewonnen'] = 0;
                $stand[$row[2]]['verloren'] = 0;
                $stand[$row[2]]['gelijk'] = 0;
                $stand[$row[2]]['punten'] = 0;
                $stand[$row[2]]['logo'] = '';
            }
    ...
    // Check select voor nog niet gespeelde wedstrijden
    if ($result3 == false){
        $message .= melding(mysqli_errno($link) . ': ' . mysqli_error($link),0);
    }
    else {
